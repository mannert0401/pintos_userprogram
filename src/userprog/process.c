...
process_execute (const char *file_name) 
{
  ...
  /* parsing file name and get thread name */
  char *next_ptr;
...
{ 
  ...
  /* we must free resources because memory leak */
  palloc_free_page (file_name);
  free(parse);
  free(f2);
...
load (const char *file_name, void (**eip) (void), void **esp) 
{
  ...
  /* Setting struct thread's run file flag using by this file. */
  t->run_file = file;
